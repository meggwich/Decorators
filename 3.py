...
def logger(old_function):
    def new_function(*args, **kwargs):
        ...
        with open('main.log', 'a') as f:
            f.write(f"{datetime.datetime.now()} - вызвана функция {old_function.__name__} с аргументами {args, kwargs}. Возвращаемое значение: {result}\n")
        return result
    return new_function
...
@logger_with_path('application.log')
def run(self, browser, url, number_of_vacancies, waiting_time):
    ...
    while len(self.vacancies_data) < number_of_vacancies:
        ...
        vacancies_tags = div_tag.find_all("div", {
